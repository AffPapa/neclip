...
final class ApplicationModeContractTests: XCTestCase {
    ...
    func testReleaseContractKeepsNeClipInMenuBarAndOutOfDock() throws {
        ...
        XCTAssertFalse(statusBar.contains("Действия с верхним элементом"))
        XCTAssertFalse(statusBar.contains("toggleFirstResultPin"))
    ...
    func testReleaseScriptFailsClosedAndAvoidsDeepSigning() throws {
        ...
        XCTAssertTrue(script.contains("notarytool submit \"$NOTARY_ZIP\""))
        ...
        XCTAssertTrue(script.contains("ditto -c -k --sequesterRsrc --keepParent \"$APP\" \"$ZIP\""))
        XCTAssertTrue(script.contains("ZIP_APP=\"$ZIP_VERIFY_DIR/NeClip.app\""))
        XCTAssertTrue(script.contains("stapler validate \"$ZIP_APP\""))
        XCTAssertTrue(script.contains("ZIP_SHA=$(awk"))
        XCTAssertTrue(script.contains("NeClip-${VERSION}.zip.sha256"))
