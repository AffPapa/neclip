...
- A later sandboxed `codesign` invocation reported an invalid signature after the host attached `com.apple.provenance` metadata. Repeating the same verification from the permitted filesystem context passed: the installed bundle passed `codesign --verify`, stapled-ticket validation and Gatekeeper; the executable SHA-256 remained `52bcc56e8197a10afd8096fe085ae1a44957a6008a803b46094858f636ec639e`. This resolves the sandbox-only verification error.
